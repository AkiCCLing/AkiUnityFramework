...
namespace AkiFramework.Editor
{
    ...
    public static class UIScriptCreator
    {
        ...
        {
            var scriptDir = NormalizeAssetFolderPath(GetScriptOutputDirectory(fileName), "Assets/GameApp/Scripts");
        ...
        public static void DoCreate(string fileName, string template)
        {
            if (Selection.objects.Length > 0)
            {
                ...
                {
                    var scriptDir = NormalizeAssetFolderPath(GetScriptOutputDirectory(fileName), "Assets/GameApp/Scripts");
                    var bindingDir = NormalizeAssetFolderPath(GetBindingOutputDirectory(), "Assets/GameApp/Scripts/UI/Binding");
                    ...
                    {
                        QueuePrefabAttach(prefabPath, $"GameApp.UI.{prefab.name}");
                    }
        ...
        private static void GenerateBindingPartialScript(GameObject prefab, string currentDir)
        {
            ...
            bindingScript.AppendLine("namespace GameApp.UI");
            bindingScript.AppendLine("{");
        ...
        private static void AddScriptToPrefab(GameObject prefab, string scriptName)
        {
            ...
            {
                QueuePrefabAttach(queuedPrefabPath, $"GameApp.UI.{scriptName}");
            }
            return;
            // 多次尝试查找脚本类型，因为编译可能需要时间
            Type scriptType = null;
            string fullScriptName = $"GameApp.UI.{scriptName}";
